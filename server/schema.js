...
MongoClient.connect('mongodb://admin:admin@ds249545.mlab.com:49545/graphql_redux', function(err, db) {
    ...
    addProduct: {
      ...
      resolve: function(rootValue, args) {
        ...
        return productsCollection.count().then(length => {
          ...
          product.id = `${length + 1}`;
          return productsCollection.insert(product).then(_ => product);
        })
      }
    },
    editProduct: {
    ...
    removeProduct: {
      ...
      resolve: function(rootValue, { id }) {
        return productsCollection.remove({
          id
        }).then(_ => productsCollection.find().toArray());
      }
    }
  }
});
