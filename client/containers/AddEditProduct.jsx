...
class AddEditProduct extends React.Component {
    componentWillMount() {
        ...
        if (params.id) {
            ...
            let that = this;
            getProduct(`{product(id:"${params.id}"){_id, name, description, quantity, cost, image}}`).then(function () {
    ...
    addOrEdit(data) {
        ...
        if (params.id !== undefined) {
            ...
            if (data.description)
                dataStr += `description: "${data.description}", `;
            if (data.quantity)
                dataStr += `quantity: ${data.quantity}, `;
            if (data.cost)
                dataStr += `cost: ${data.cost}, `;
            if (data.image)
...
const mapStateToProps = (state) => {
    return {product: state.products.get("product"), fetching: state.products.get("fetching")}
};
